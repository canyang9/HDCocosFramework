...
interface QgNativeAdItem {
  adId: string
  title: string
  desc: string
  icon: string
  imgUrlList: Array<any>
  logoUrl: string
  clickBtnTxt: string
  creativeType: number
...
interface QgBannerAd {
  ...
  onSize: (callback: Function)=>void
  offSize: (callback: Function)=>void
}
...
declare interface QgInnerAudioContext {
  src: string
  loop: boolean
  volume: number
  duration: number
...
declare namespace qg {
  export function hasShortcutInstalled(obj: { success: (status)=>void })
  export function installShortcut(obj: { message?: string, success?: Function, fail?: Function, complete?: Function })

  export function applyUpdate(): void

  export function share(obj: { success: ()=>void, fail: (errmsg, errcode)=>void, cancel: ()=>void, complete: ()=>void })

  export function onUpdateReady(callback: Function): void
   ...
   */
  export function setStorageSync(obj: {key: string, default: string}): void;
  ...
  export function showDialog(obj: { title?: string, message?: string, buttons?: Array<{ text: string, color: string }>, 
   ...
   */
  export function  createRewardedVideoAd(object:any):QgVideoAd;
  /**
   * **确保广告服务已经初始化完毕**
   * 创建插屏广告组件，同一个 posId，如果已经创建，并且未 destroy，会复用之前的对象
   */
  export function createInsertAd(object:any):QgInsertAd;
  /**
   * **确保广告服务已经初始化完毕**
  ...
  /** returns 文件目录存在返回字符串'true'/'false'， 参数错误返回number 202，I/O错误返回number 300 */
  export function accessFile(obj: { uri: string, md5?: string })

  export function copyFile(obj: { srcUri: string, dstUri: string, success?: (uri)=>void, fail?: (data, code)=>void, complete?: Function })

  export function moveFile(obj: { srcUri: string, dstUri: string, success?: (uri)=>void, fail?: (data, code)=>void, complete?: Function })

  /** success?: (data)=>void 其中data包含uri、length和lastModifiedTime */
  export function getFileInfo(obj: { uri: string, success?: (data)=>void, fail?: (data, code)=>void, complete?: Function  })

  export function writeFileSync(obj: { uri: string, text: any, encoding?: string, position?: number })

  export function unzipFile(obj: { srcUri: string, dstUri: string, success?: (uri)=>void, fail?: (data, code)=>void, complete?: Function })
